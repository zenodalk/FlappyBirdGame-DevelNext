...
{
    /**
     * @event tubeGenerator.action 
     **/
    function doTubeGeneratorAction(ScriptEvent $event = null)
    {    
        global $gameRunning;

        if ($gameRunning) {
            $event->sender->interval = rand(2000, 4000);
        
            $bTube = $this->create('Proto.bottomTube', null, 464, 268 + rand(-30, 30));
            $tTube = $this->create('Proto.topTube', null, 464, -184 + rand(-30, 30));

            $this->instances('Proto.background')->toFront();
            $this->scorePanel->toFront();
        }
    }

    /**
     * @event coinGenerator.action 
     **/
    function doCoinGeneratorAction(ScriptEvent $event = null)
    {
        global $gameRunning;

        if ($gameRunning) {
            $event->sender->interval = rand(500, 2000);
            
            $type = 'coin';
            
            // в 1 из 4 случаев монетка будет большой
            if (rand(1, 4) == 4) {
                 $type = 'bigCoin';   
            }
            
            $coin = $this->create("Proto.$type", null, 464, 250 + rand(-20, 20));
        }
    }

    /**
     * @event scoreUpdater.action 
     **/
    function doScoreUpdaterAction(ScriptEvent $event = null)
    {
        $this->scoreLabel->text = Score::get('global');      
    }
}
